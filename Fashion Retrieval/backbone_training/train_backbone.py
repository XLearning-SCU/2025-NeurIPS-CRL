...
if __name__ == "__main__":
    ...
    mlp_model = AttributeProjectionModel(in_dim=text_embedding.shape[2], out_dim=1024, n_attr=cfg.DATA.NUM_ATTRIBUTES).to(device)
    ...
    optimizer_mlp = torch.optim.Adam(mlp_model.parameters(), lr=cfg.SOLVER.MLP_LR)
    ...
    for epoch in range(cfg.SOLVER.EPOCHS):
        print(f"Epoch {epoch}, lr_clip: {optimizer_clip.param_groups[0]['lr']}, lr_mlp: {optimizer_mlp.param_groups[0]['lr']}")
        ...
        for idx, (x, p, n, a) in tqdm(enumerate(train_loader)):
            ...
            x_sim = torch.bmm(x_img.unsqueeze(1), text_embedding[a]).squeeze(1)
            p_sim = torch.bmm(p_img.unsqueeze(1), text_embedding[a]).squeeze(1)
            n_sim = torch.bmm(n_img.unsqueeze(1), text_embedding[a]).squeeze(1)
        ...
        if (epoch+1) % cfg.SOLVER.EVAL_STEPS == 0:
            ...
            q_feats, q_values = extract_features(clip_model, mlp_model, valid_query_loader, text_embedding, cfg.DATA.NUM_ATTRIBUTES, preprocess, device)
            print("Forwarding candidate images...")
            c_feats, c_values = extract_features(clip_model, mlp_model, valid_candidate_loader, text_embedding, cfg.DATA.NUM_ATTRIBUTES, preprocess, device)
