...
class MetaLoader(object):
    def __init__(self, root, dataset):
        self.data = json.load(open(os.path.join(root, dataset, 'meta.json')))[dataset]
...
class ImageLoader(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return img, c, gdtruth, self.filenamelist[int(path)].strip().split('/')[-1].split('.')[
        ...
        #         0]  # imgdata, condition, groundtruth, imgid(for retrieval use)
        # else:
        #     print(f"Data or label is None at index {index}")
        #     print(os.path.join(self.root, self.base_path, self.filenamelist[int(path)]))
        #     return None
